...
const {data:carousels} = await useAsyncData('getCarousel',async()=>{
    const res = await useSupabase().from('carousel').select().limit(10)
    return res.data
})
// 修改轮播图状态
const changeIsActive = async(item)=>{
    // 修改数据库
    const res = await useSupabase().from('carousel').update({is_active:item.is_active}).eq('id',item.id)
}
// 删除轮播
